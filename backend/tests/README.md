...
- **`meta/test_lessons_enforced.py` 全绿** —— 说明没有新的零断言 / 重言式 /
  吞异常 / 条件断言混进来。这个文件抓过本轮我自己写的多条弱用例。

## 怎么跑

```bash
cd backend
python -m pytest tests/ -q -p no:randomly        # 全量（约 24 分钟）
python -m pytest tests/meta/ -q                  # 只跑套件自检（约 1 分钟）
...
**对套件与代码库本身的约束**。不测某个功能，测的是「测试有没有资格被信任」「有没有模块写了没接线」「已登记缺陷还欠几个」。审计从这三个文件开始看。

| 文件 | 用例 | 测什么 |
|---|---:|---|
| `test_invariants.py` | 6 | 系统级**不变量**（不是用例） |
| `test_known_defects.py` | 23 | 每条一个 `xfail(strict=True)` 用例。 |
...
LLM agent：意图路由、无 LLM 回退、工具守卫、批评器阈值。

| 文件 | 用例 | 测什么 |
|---|---:|---|
| `test_agent_critic.py` | 12 | OverfitCritic 阈值逻辑测试 |
| `test_agent_fallback.py` | 10 | FallbackOrchestrator 与 QuantTools 测试（无 LLM 模式） |
...
回测与绩效：撮合账务、组合构造、绩效公式、风险报告、过拟合统计。
...
| `test_backtest_plot_fidelity.py` | 2 | 回测曲线保留且真实（研究用，非伪动画） |
...
数据层：数据集注册、IS/OOS 切分、PIT 存储、市场日历、行情源、市场状态。
