...
### 3.0c 核实过程中发现的新缺陷（2026-07-24）

> 在复现 S1-S8 并跑全量回归时，`test_chat_returns_dsl` 出现**顺序敏感的偶发失败**
> （单跑必过、全量套件偶发失败）。深挖根因得到一条**报告未提及**的真实缺陷。

| ID | 严重 | 问题 | 后果 | 去向 |
|----|------|------|------|------|
| **R-N1** | 🟡 | **GP 种子/变异生成仍用全局 `random`**（`gp_engine.generate_random_alpha`、`alpha_workflows._generate_diverse_seeds`、`mutations.*`）——E4 当年只把 `PopulationEvolver` 改为实例级 RNG，这三处上游/变异辅助漏改 | GP 输出**跨进程/跨调用不可复现**（相同 hypothesis 两次运行结果可不同）；测试顺序敏感偶发失败；且与 Task 6.5 的 RunManifest 可复现性目标**直接冲突**——重放依赖确定性生成 | Task 6.5 |

**处置**：
- 立即：`test_chat_returns_dsl` 加固定 `random.seed()`，消除 CI flaky（已完成）
- 根治（并入 Task 6.5）：将实例级 `random.Random(seed)` 一路穿透到
  `generate_random_alpha` / `_generate_diverse_seeds` / `mutations.*`，使 GP 全链路
  确定性——这是 RunManifest「相同 manifest 重放误差为零」的前置条件

**方法论教训（记录以自省）**：核实本身引入过一次事故——用 `git stash` 建立基线对照后，
`stash pop` 因 `.pyc`/`.db` 二进制冲突未完整还原，误 `stash drop` 一度丢失全部源码
修复；靠 drop 时打印的 stash 提交哈希 `git checkout <hash> -- <path>` 完整找回。
**教训**：(1) `.pyc`/`*.db` 应在 `.gitignore`（Task 6.2 已列）避免污染 stash；
(2) 建立基线对照应优先用 `git worktree` 而非 `stash`，避免二进制冲突与丢改动风险。
...
- **审计谱系**（RESEARCH_OPERATING_MODEL §2 落地）：GP 每代每个个体的完整评估
  ...
  可完整回放"它如何被选出"
- **R-N1 全链路确定性（体检 §3.0c 新发现，前置条件）**：将实例级 RNG 穿透到
  `generate_random_alpha` / `_generate_diverse_seeds` / `mutations.*`，消除全局
  `random` 依赖——否则"相同 manifest 重放误差为零"无法达成
...
## 四·补、数据集聚焦美股 + 加密（2026-08-17）

系统主力市场确定为**美股 + 加密货币**，据此调整数据集注册表：
- **搁置** A 股（3）+ 港股（1）：定义原地保留在 `dataset_registry._SHELVED_SPECS`
  （供未来恢复），导入时从 `_SPECS` 弹出 → 不注册/不列举/不可加载（请求时给"已搁置"提示）。
  搁置理由：A 股散户程序化实盘不可行 + 涨跌停未建模；港股非主力。
- **新增 `us_broad_large`（95 只跨行业宽基）**：覆盖全部 11 个 GICS 行业（分布：IT 16、
  金融 12、医疗 12、工业 10、可选消费 10、必需消费 9、通信 8、能源 5、材料 5、公用 4、
  地产 4）。填补"综合美股 universe"缺口——**跨行业截面研究/行业中性化/因子正交化在此才有意义**。
  同步补全 `sector_mapper` 静态 GICS 映射（52 只新增，0 缺失）。
- **扩充加密**：crypto_major 12→23（含 TON/NEAR/BCH/DOGE 等主流），crypto_alt 16→27
  （L2/新 L1/AI/DeFi/Gaming 主题更全）。
- 活跃数据集：5 美股（含宽基）+ 2 加密 = 7 个；router 字段描述/列举顺序同步更新。

> **仍存的美股短板**（非本次范围）：非宽基级全市场（如 S&P 500 全量）；除权/停牌/日历事件未建模；
> **无券商接入**（不能接实盘，需 IBKR/Alpaca；加密可走 ccxt create_order，均未建）；幸存者偏差。
