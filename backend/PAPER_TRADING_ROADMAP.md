...
**制定日期：** 2026-07-17
**前置状态：** DEV_ROADMAP Phase 0–4 已完成并验收（v4.0）；Phase 5 未开始
...
```
Phase 5  ── 在线化与生命周期（现有路线图收尾）        ~14 天
Phase 6  ── 基础设施加固（工程严谨性）                ~8 天
Phase 7  ── PaperBroker 与模拟交易循环（核心新增）    ~10 天
Phase 8  ── PIT 数据层与验证运营（持续运行）           ~5 天启动 + 长期
─────────────────────────────────────────────────────
专注开发总计约 37 个工作日（7-8 周）
之后进入 ≥ 3 个月的 paper trading 验证期（系统自动运行，人工每周复盘）
```

依赖关系：Phase 6 的 6.1/6.2 应**最先做**（消除静默损坏源），可与 Phase 5 并行；
Phase 7 依赖 Phase 5 的调度器与生命周期状态机；Phase 8 依赖 Phase 7。

---

## 二、Phase 5 — 在线化与生命周期（现有路线图，~14 天）

> 任务定义见 DEV_ROADMAP v4.0，此处仅补充验收标准与顺序调整。

| Task | 内容 | 估时 | 验收标准 |
|------|------|------|---------|
| 5.2 | 生命周期状态机（建议**先做**，5.1 依赖状态定义）| 3 天 | `CANDIDATE→VALIDATED→PAPER→ACTIVE→DECAYING→RETIRED` 全部状态流转有单元测试；非法流转被拒绝 |
| 5.1 | AlphaMonitor 在线监控 + `alpha_ic_history` 表 | 5 天 | `update()` 幂等（同日重复调用不重复记账）；连续 N 日负 IC 触发 DecayAlert；`get_dashboard()` 返回全部 PAPER/ACTIVE 因子的滚动 IC-IR |
| 5.3 | APScheduler 调度框架 | 3 天 | 任务持久化到 SQLite（重启不丢）；错过的任务按 misfire 策略补跑或跳过并告警；时区显式配置 |
| 5.4 | API 端点扩展（dashboard / ic_history / retrain / status / walk_forward）| 3 天 | 5 个端点集成测试；PATCH status 走状态机校验 |

**Phase 5 验收**：新增测试全绿 + 现有 342 项回归无失败 + 手动触发一次完整
「监控更新 → 衰减告警 → 状态流转」链路演示。

---

## 三、Phase 6 — 基础设施加固（~8 天）
...
### Task 6.2 🔴 迁出 OneDrive + SQLite 加固 — 0.5 天
...
| M2 | Phase 5 全部 + 验收 | 第 4 周 |
| M3 | Phase 6 剩余（CI + 黄金测试 + 可复现性）| 第 5 周 |
| M4 | Phase 7 全部 + 20 日历史回放对账通过 | 第 7 周 |
| M5 | Phase 8 启动，**系统进入每日自动 paper trading** | 第 8 周 |
| M6 | 首批因子完成 60 交易日 PAPER 验证，出具首份验证报告 | M5 + 3 个月 |
...
| B2（组合权重 OOS 拟合）尚未修复 | combined_metrics 乐观偏差 | 已在 DEV_ROADMAP 标注；建议并入 Phase 5 顺手修复（改为 IS 拟合，~0.5 天）|
...
*规划版本 v1.0 | 2026-07-17 | 前置：DEV_ROADMAP v4.0（Phase 0-4 完成）*
