...
(Physically moved to core/agent/ from core/ml_engine/)
"""
...
class AlphaAgent:
    ...
    def run(self, hypothesis: str, dataset: Dict[str, pd.DataFrame]) -> ReasoningLog:
        ...
        passed_any = False
        for dsl in dsls:
            result_dsl, metrics = self._evaluate_and_refine(dsl, hypothesis, dataset, log)
    ...
    def _generate_dsls(self, hypothesis: str) -> List[str]:
        ...
        return dsls if dsls else ["rank(ts_delta(close,5))", "zscore(ts_mean(returns,10))"]
    ...
    def _validate_and_fix(self, dsl: str) -> Optional[str]:
        for attempt in range(3):
            try:
                ...
                try:
                    dsl = json.loads(resp).get("dsl", dsl)
                except Exception:
                    return None
        return None
    ...
    def _evaluate_and_refine(
        ...
        for refine in range(self._max_refine):
            ...
            log.add_change(old_dsl=valid_dsl, new_dsl=fixed, reason=change_r, metrics=new_metrics)
            valid_dsl = fixed
            metrics   = new_metrics
