...
### S.4 一条修好的教训会**换个地方**复发（Phase S，2026-09-22）

B-2 修的是"`n_days` 被端点静默忽略"。做 Phase S 的三段切割时，冻结窗口兑现不了，
我的第一版实现把**选择段钉在下限上**（`n_test = n - min_selection_days`）。
后果：`n_days=400` 与 `n_days=900` 切出**同样大**的研究样本 —— 多给的数据
全进了冻结段。端点没有忽略 `n_days`，B-2 的那条用例照样绿，可"给更多数据"
在研究侧依然毫无变化。是 B-2 的回归用例（比较两种 `n_days` 用掉的天数）把它抓住的。

改法：封顶用**份额**（`s_max_test_share`）而不是下限反推，保持"数据变多 → 研究样本
也变多"的单调性；交界处允许一次性缩小（换来一个真正固定的样本外窗口），并写进文档。

- **判据**：给一个资源加上限时，先问"它会不会让某个输入参数从此不再影响输出"。
  会的话，那个参数的既有回归用例就是你的检查 —— 跑它，别只跑新写的那些。
...
## W. 通过数在弱套件上不是证据；先测**检出能力**，再谈"回归通过"

**触发**：我一边知道套件的断言强度只有 37.5%（`leak_filter` 隔离版实测），
一边把"680 passed / 0 failed"当作健康信号报出去。用户一句话点破：
「你测试基本全是错的，你跑 full regression 的意义？」

这就是 §R 里我自己写下的"**把通过率当覆盖率**"，只是换了个更隐蔽的形态：
上一次是"10/10 passed 不证明没跑的没错"，这一次是
"**680 passed 不证明这 680 条能发现错**"。

### 三种测试信号的效力，不可混为一谈

| 信号 | 能回答什么 | 不能回答什么 |
|------|-----------|-------------|
| 通过数（N passed） | 「以本套件**能察觉**的方式，没有变坏」 | 本套件能察觉多少 |
| 行覆盖率 | 「这些代码被**执行**过」 | 执行时有没有被**断言** |
| **变异击杀率** | 「改坏了**会不会被发现**」 | 未被变异算子覆盖的错误类型 |

前两个都可以在断言全空的套件上刷到很高。**只有击杀率直接度量检出能力。**

### 规则
1. **未知击杀率的模块，其测试结果不得作为正确性证据引用**。可以说"没有新增崩溃"，
   不能说"验证通过"。
2. **报告顺序固定**：先报击杀率与未测量范围，再报通过数；通过数只作为
   "本次变更是否打破了已有断言"的**变更影响检查**。
3. **改了算法就必须跑回归** —— 但那是查"有没有测试断言了旧数值"，
   属于影响面排查，不是健康证明。两者说法要分开。
4. 建 `MUTATION_LEDGER.md` 台账：口头报过的数字不可信（本会话已有 6 个被推翻），
   一律以台账为准，并标注 ✅已测 / ⚠️作废 / ⬜从未测量。
5. **先验证测量工具本身，再信它给的数**。度量工具的缺陷不会报错，只会
   **静默缩小分母**，把"没测过"伪装成"测过且通过"。

### 度量工具静默失效的两个真实形态（都是我自己写的工具）

**形态一：变异器根本没生效，却被记成"无可变异"。**

```python
mutated = line[:m.start()] + re.sub(pat, rep, m.group(0), count=1) + line[m.end():]
```

`m.group(0)` 不包含 lookaround 消耗的字符。把它单独送进 `re.sub`，
**正向**后顾断言 `(?<=[\w\)\]])` 前面没有字符，必然失配 → 原样返回 →
`mutated == line` → 计入"无可变异"。于是 `*` `+` `-` 三个算术变异器
**一次都没生效过**，而 `<` `>` 用的是**负向**断言（前面没字符 = 满足），照常工作。

**症状是可读出来的**：存活列表清一色是比较符，一个乘除都没有——
真实代码不可能只在比较符上有盲区。`transaction_cost` 129 个候选行只有 22 行
被变异，`dollar_vol = volume * prices`、`max_usd = adv * adv_cap_pct`、
`trade_usd = delta_w * portfolio_val` 这些**算钱的乘法一个都没测过**。

**形态二：枚举策略偏心。** 每行只取第一个匹配的变异器，而变异器表里比较符排在
算术符前面，于是 `shares = abs(dw) * val / p if p > 0 else 0.0` 这类行
**永远只测 `>`，乘法永远轮不到**。改成"每行每个变异器各算一个变异点"后，
`transaction_cost` 22 → 42、`risk_gate` 22 → 42，**变异面直接翻倍**。

修完这两条，先前所有击杀率——**包括我刚宣布"已达标"的 risk_gate 54.5%**——
都是在不到一半的变异面上算出来的，一律作废重测。

- **判据一**：想说"测试通过了"之前，先答一句——「这套测试对这个模块的击杀率是多少？」
  答不上来，就只能说"没有被现有断言拦下"。
- **判据二**：拿到击杀率之前，先答一句——「这个数的**分母**是怎么来的，
  它覆盖了哪几类错误？」分母不可解释，分子就没有意义。
  具体做法：跑一次**只出计划不执行**的干跑，人眼核对"该被变异的行有没有在列表里"。
